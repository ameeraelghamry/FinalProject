...
const { sendConfirmationEmail, sendDeclinedEmail } = require('./utils/emailService');
...
app.use('/uploads', express.static('uploads')); // Add static middleware for uploads
...
// Cookie consent session handler
app.get('/api/cookie-consent/session_:id', (req, res) => {
  res.json({ status: 'ok' });
});
...
// Handle form submission to pending page
  ...
  try {
    ...
    if (!bookingRef) {
      return res.status(404).render('404', {
        user: req.session?.user,
        message: 'Booking reference not provided',
        requestedUrl: req.originalUrl
      });
    }
    ...
    res.status(500).render('404', {
      user: req.session?.user,
      message: 'Error loading pending page',
      requestedUrl: req.originalUrl
    });
  }
});
